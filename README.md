...
# 不再维护，请使用 [ianchb/debian-sheng](https://github.com/ianchb/debian-sheng)构建
...
4.内核编译完成后在Action左侧选择发行版（如Debian，Ubuntu尚未完整测试），点击`Run workflow`在输入你编译好的内核版本号，选择版本，构建类型等，点击绿色的`Run workflow`开始构建rootfs，构建完成后的成品可在release查看,如果成品未在release上出现为rootfs的大小超过了Github的限制需在Action下载
